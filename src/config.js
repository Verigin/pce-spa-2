...
export const switchEnvironment = function (nextEnvironmentName) {
  const nextConfig = environments.find(
    environment => environment.name === nextEnvironmentName
  )
  if (!nextConfig || !nextConfig.name) { console.error('Environment not found: ' + nextEnvironmentName) }
  // Copy all into config object
  for (const k in nextConfig) { config[k] = nextConfig[k] }
